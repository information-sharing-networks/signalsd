...
3. Promotes the staging image to production in ECR — re-tags `staging-<git-sha>` as
   `<git-sha>` and `latest` without rebuilding. This guarantees the image deployed to
   production is bit-for-bit identical to what was tested in staging.
4. Fetches `DATABASE_URL` from Secrets Manager and runs database migrations (`goose up`)
...
- Builds and pushes the Docker image (production promotes the staging image instead of rebuilding)
- Uses staging Secrets Manager secrets and variables
...
`ENVIRONMENT`, `LOG_LEVEL`, `PUBLIC_BASE_URL`, `ALLOWED_ORIGINS` and the optional tuning
variables are set via `RuntimeEnvironmentVariables` and rebuilt on every deployment. To
change them:

1. Update the GitHub variable in repository settings
2. Trigger a deployment (push a new tag for prod, merge to main for staging, or use
   `workflow_dispatch`)

### Application secrets (DATABASE_URL, SECRET_KEY)

These are stored in AWS Secrets Manager and fetched by App Runner at container startup. To
rotate a secret:
...
1. Update the value in Secrets Manager:
   ...
   aws secretsmanager put-secret-value \
     ...
     --region eu-west-1
   ```
2. Trigger a redeployment so App Runner restarts containers with the new value:
   ```bash
   # Push a new tag for production
   git tag v1.2.4 && git push --tags

   # Or trigger manually via workflow_dispatch in the GitHub Actions UI
   ```

Unlike Cloud Run (where `gcloud run services update --set-env-vars` can change variables
without redeploying the image), App Runner's configuration is always set alongside the image
reference. This means every config change goes through the CD workflow — intentional, keeping
config and code changes in the same audit trail.
...
| Application secrets | GitHub repository secrets | AWS Secrets Manager |
