...
export OWNER=nick@gmail.com
export PASSWORD=12345678901
export ISN_SLUG=perf-test
export SIGNAL_TYPE=perf-test-validated
export SEM_VER=1.0.0
export BATCH_SIZE=1
...
run_test() {
    ...
    env BASE_URL="$BASE_URL" AUTH_TOKEN="$AUTH_TOKEN" OWNER="$OWNER" PASSWORD="$PASSWORD" \
        ISN_SLUG="$ISN_SLUG" SIGNAL_TYPE="$SIGNAL_TYPE" SEM_VER="$SEM_VER" \
        BATCH_SIZE="$BATCH_SIZE" NUM_BATCHES="$NUM_BATCHES" \
        ./signal-loader > "$log_file" 2>&1
