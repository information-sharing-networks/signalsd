...
func (q *Queries) GetLoadedSignalsSummaryByBatchID(ctx context.Context, signalBatchID uuid.UUID) ([]GetLoadedSignalsSummaryByBatchIDRow, error) {
	rows, err := q.db.Query(ctx, GetLoadedSignalsSummaryByBatchID, signalBatchID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
