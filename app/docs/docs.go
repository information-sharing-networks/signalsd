    ...
    "paths": {
        ...
        "/admin/accounts/{account_id}/enable": {
            "post": {
                ...
                "description": "**Administrative endpoint to re-enable previously disabled accounts.**\nSets account status to ` + "`" + `is_active = true` + "`" + ` but does NOT create new tokens.\n\n**Use Cases:**\n- **Restore Access**: Re-enable accounts after security incident resolved\n- **End Suspension**: Restore access after policy violation addressed\n- **Employee Return**: Restore access for returning staff\n- **Appeal Granted**: Restore access after successful appeal\n\n**Post-Enable Steps Required:**\n- **Service Accounts**: Must re-register via ` + "`" + `/api/auth/register/service-accounts` + "`" + ` (same client_id, new credentials)\n- **Web Users**: Can immediately log in again via ` + "`" + `/auth/login` + "`" + `\n\n**Note:** This only changes account status - it does not create new authentication credentials.\nOnly owners and admins can enable accounts.",
                "tags": [
                    "Site admin"
                ],
